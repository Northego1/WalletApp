...
class WalletOperationController:
    ...
    async def operation(
        ...
        try:
            operation_use_cases = self.use_case_operation_dict[
                wallet_request_dto.operationType
            ]
            wallet_balance_dto = await operation_use_cases.operation(
                wallet_id=wallet_id, amount=wallet_request_dto.amount
            )
