...
class WalletDepositUseCase:
    ...
    async def operation(self: Self, wallet_id: uuid.UUID, amount: float) -> WalletBalanceDto:
        async with self.uow:
            wallet = await self.uow.repository.get_wallet_for_update(wallet_id=wallet_id)
            if not wallet:
            ...
            await self.uow.repository.update_wallet_balance(wallet_id=wallet_id, new_balance=wallet.balance)
