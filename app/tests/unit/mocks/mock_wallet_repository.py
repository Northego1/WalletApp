...
mock_wallet_repository = cast(
    WalletRepositoryProtocol,
    Mock()
)

MockWalletRepository = Mock(spec=WalletRepositoryProtocol, return_value=mock_wallet_repository)

mock_wallet_repository.get_wallet_by_id = AsyncMock(
    side_effect=get_wallet_by_id
)

mock_wallet_repository.update_wallet_balance = AsyncMock(
    return_value=None
)


mock_wallet_repository.get_wallet_for_update = AsyncMock(
    side_effect=get_wallet_by_id
)
